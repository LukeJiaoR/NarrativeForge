...
class TestTaskService(unittest.TestCase):
    ...
    @unittest.skipUnless(
        os.getenv("NARRATIVEFORGE_TEST_REDIS_HOST"),
        "NARRATIVEFORGE_TEST_REDIS_HOST not set",
    )
    def test_real_redis_recovers_interrupted_cross_post_state(self):
        """真实 Redis 中的遗留发布状态必须在恢复后保留视频并进入失败终态。"""
        state = RedisState(
            host=os.environ["NARRATIVEFORGE_TEST_REDIS_HOST"],
            port=int(os.getenv("NARRATIVEFORGE_TEST_REDIS_PORT", "6379")),
            db=int(os.getenv("NARRATIVEFORGE_TEST_REDIS_DB", "15")),
        )
    ...
    @unittest.skipUnless(
        ...
        "NARRATIVEFORGE_RUN_INTEGRATION_TESTS not set",
    )
