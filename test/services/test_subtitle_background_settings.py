...
class TestSubtitleBackgroundSettings(unittest.TestCase):
    ...
    def test_detects_font_without_chinese_glyphs(self):
        ...
        latin_font = next(
            (path for path in latin_font_candidates if path.is_file()),
            None,
        )
        if latin_font is None:
            self.skipTest("no Latin-only system test font is available")
        unicode_font = utils.resolve_font_path("system-default")

        self.assertFalse(
            video.subtitle_font_supports_text(
                str(latin_font), "人工智能改变生活"
            )
        )
        self.assertTrue(
            video.subtitle_font_supports_text(
                unicode_font, "人工智能改变生活"
            )
        )
        self.assertTrue(
            video.subtitle_font_supports_text(
                str(latin_font), "Artificial intelligence"
            )
        )
    ...
    def test_wrap_text_keeps_closing_punctuation_with_text(self):
        ...
        """
        font_path = utils.resolve_font_path("system-default")
