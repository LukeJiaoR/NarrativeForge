...
RUN_INTEGRATION_TESTS = os.environ.get("NARRATIVEFORGE_RUN_INTEGRATION_TESTS", "").lower() in {
    "1",
    "true",
    "yes",
}
...
class TestScriptPromptOptions(unittest.TestCase):
    ...
    def test_generate_terms_describes_visible_context_without_repeating_subject(self):
        """检索提示应适用于任意歧义主题，而不是依赖某个技术产品特例。"""
        captured = {}

        def fake_generate_response(prompt):
            captured["prompt"] = prompt
            return '["designer reviewing interface", "team testing mobile app"]'
        ...
        with patch.object(
            ...
            result = llm.generate_terms(
                video_subject="Apple",
                video_script="A design team reviews a software product.",
                amount=2,
            )

        self.assertEqual(
            result,
            ["designer reviewing interface", "team testing mobile app"],
        )
        self.assertIn("person, action, object, place, or event", captured["prompt"])
        self.assertIn("disambiguate proper nouns and homonyms", captured["prompt"])
        self.assertIn(
            "do not repeat the title, brand, product, person, or main subject",
            captured["prompt"],
        )
        self.assertNotIn("technology brand", captured["prompt"])
...
@unittest.skipUnless(
    ...
    "NARRATIVEFORGE_RUN_INTEGRATION_TESTS and ANTHROPIC_FOUNDRY_API_KEY not set",
)
