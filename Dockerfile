...
WORKDIR /NarrativeForge

# 设置/NarrativeForge目录权限为777
RUN chmod 777 /NarrativeForge

ENV PYTHONPATH="/NarrativeForge"
...
# docker build -t narrativeforge .

# 2. Run the Docker container using the following command
## For Linux or MacOS:
# docker run -v $(pwd)/config.toml:/NarrativeForge/config.toml -v $(pwd)/storage:/NarrativeForge/storage -p 127.0.0.1:8501:8501 narrativeforge
## For Windows:
# docker run -v ${PWD}/config.toml:/NarrativeForge/config.toml -v ${PWD}/storage:/NarrativeForge/storage -p 127.0.0.1:8501:8501 narrativeforge
