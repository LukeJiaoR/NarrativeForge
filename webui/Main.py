...
# 避免依赖中的同名 app 包遮蔽 NarrativeForge 自己的 app 包。
...
st.set_page_config(
    ...
    menu_items={
        "About": "# NarrativeForge\nAgent-first orchestration for auditable "
        "narrative media workflows.",
    },
)


# Streamlit 1.59 会在页面右上角默认展示 Deploy、skills nudge 等平台入口。
# NarrativeForge 是面向终端用户的本地工具，这些入口会造成顶部大块空白，
...
def _render_brand(available_update: str | None = None):
    ...
    st.markdown(
        f"""
        <h1 class="nf-brand">
            <span class="nf-brand__name">NarrativeForge</span>
            <span class="nf-brand__version">v{html.escape(str(config.project_version))}</span>
            {update_link}
        </h1>
        """,
