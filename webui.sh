...
NARRATIVEFORGE_WEBUI_HOST="${NARRATIVEFORGE_WEBUI_HOST:-127.0.0.1}"
NARRATIVEFORGE_WEBUI_PORT="${NARRATIVEFORGE_WEBUI_PORT:-8501}"
...
find_available_port() {
  WEBUI_HOST="$NARRATIVEFORGE_WEBUI_HOST" WEBUI_PORT="$NARRATIVEFORGE_WEBUI_PORT" "$@" - <<'PY' 2>/dev/null
import os
import socket
import sys
...
if [ "$SELECTED_WEBUI_PORT" != "$NARRATIVEFORGE_WEBUI_PORT" ]; then
  echo "***** Port $NARRATIVEFORGE_WEBUI_PORT is unavailable, using $SELECTED_WEBUI_PORT instead. *****"
fi

NARRATIVEFORGE_WEBUI_PORT="$SELECTED_WEBUI_PORT"

echo "***** WebUI address: http://$NARRATIVEFORGE_WEBUI_HOST:$NARRATIVEFORGE_WEBUI_PORT *****"
"$@" run "$CURRENT_DIR/webui/Main.py" \
  --server.address="$NARRATIVEFORGE_WEBUI_HOST" \
  --server.port="$NARRATIVEFORGE_WEBUI_PORT" \
  --browser.serverAddress="$NARRATIVEFORGE_WEBUI_HOST" \
