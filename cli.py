...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run NarrativeForge media workflows without the WebUI.\n\n"
            "Provider settings and credentials are read from config.toml.\n"
            "Network-backed stages require their corresponding provider credentials."
        ),
        epilog="""
Examples:
...
def prepare_cli_files(params: VideoParams, stop_at: str) -> None:
    ...
    if params.subtitle_enabled and params.font_name and stop_at == "video":
        if params.font_name == "system-default":
            # Validate the platform fallback early, while preserving the sentinel
            # for the rendering service to resolve on the worker that runs it.
            utils.resolve_font_path(params.font_name)
        else:
