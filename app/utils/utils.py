...
def resolve_font_path(font_name: str | None = "system-default") -> str:
    """Resolve a bundled/user font or a portable system default.

    Third-party font binaries are intentionally not committed to the repository.
    A user-provided filename is resolved from ``resource/fonts`` first. The special
    ``system-default`` value selects a Unicode-capable font commonly available on
    macOS, Linux containers, or Windows.
    """
    normalized_name = str(font_name or "system-default").strip()
    bundled_path = Path(font_dir()) / normalized_name
    if normalized_name != "system-default":
        if bundled_path.is_file():
            return str(bundled_path)
        raise FileNotFoundError(
            f"subtitle font not found: {normalized_name!r}; "
            "place it in resource/fonts or select system-default"
        )
    ...
    raise FileNotFoundError(
        "no suitable system subtitle font was found; install a Unicode font "
        "or place a .ttf/.ttc file in resource/fonts"
    )
