"""检查 NarrativeForge 是否存在可用的新正式版本。"""
...
RELEASE_REPOSITORY: Final = os.getenv(
    "NARRATIVEFORGE_RELEASE_REPOSITORY", "LukeJiaoR/NarrativeForge"
).strip()
...
LATEST_RELEASE_PAGE_URL: Final = (
    f"https://github.com/{RELEASE_REPOSITORY}/releases/latest"
    if RELEASE_REPOSITORY
    else ""
)
...
RELEASE_CHECK_HEADERS: Final = {
    ...
    "User-Agent": "NarrativeForge-Version-Checker",
}
...
def get_available_update(current_version: str) -> str | None:
    ...
    logger.info(
        "NarrativeForge update available: "
...
class AsyncUpdateChecker:
    ...
    def poll(self, current_version: str) -> UpdateCheckSnapshot:
        ...
        with self._lock:
            ...
            worker = threading.Thread(
                ...
                name="narrativeforge-version-check",
                daemon=True,
            )
            worker.start()
    ...
    def _run_check(self, current_version: str) -> None:
        try:
            ...
            logger.exception(
                "unexpected error while checking for a NarrativeForge update"
            )
