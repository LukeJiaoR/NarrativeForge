...
2. each search term should consist of 2-6 English words and describe something
   that can be seen in stock footage: a person, action, object, place, or event.
3. you must only return the json-array of strings. you must not return anything else. you must not return the script.
4. the search terms must be related to the subject of the video.
5. reply with english search terms only.
6. do not repeat the title, brand, product, person, or main subject in every term.
7. disambiguate proper nouns and homonyms by expressing their intended context.
   For example, a product name that is also a food, animal, material, or place
   must be translated into an unambiguous visual scene rather than searched literally.
8. prefer concrete visual intent over abstract claims. Replace concepts such as
   reliability, ownership, transformation, growth, or compatibility with observable
   scenes that communicate them.
9. vary people, actions, environments, and camera subjects so the returned terms
   do not collapse into repetitive footage.
{ordering_rule}
